...
function ArchiveRail({ onClose }: { onClose?: () => void }) {
  ...
  return <aside className="archive-rail">
    ...
    <nav className="chapter-nav" aria-label="Chapter navigation">
      ...
      {filtered.map((chapter) => { const audioHref = chapter.audioSrc ?? chapter.sampleAudioSrc; const sample = !chapter.audioSrc && Boolean(chapter.sampleAudioSrc); return <div className="chapter-row" key={chapter.slug}><Link href={`/chapter/${chapter.slug}`} onClick={onClose} className={location === `/chapter/${chapter.slug}` ? "chapter-link active" : "chapter-link"}><b>{chapter.number}</b><span>{chapter.title.replace(/^Chapter\s+\d+\s*[—:-]?\s*/i, "")}</span></Link>{audioHref ? <a className="chapter-audio-link" href={audioHref} download={sample} aria-label={sample ? `Download the ${chapter.title} sample` : `Listen to ${chapter.title}`} title={sample ? "Download chapter sample" : "Listen to verified chapter audio"}><Volume2 size={14} /></a> : <span className="chapter-audio-pending" aria-label={`Audio pending for ${chapter.title}`} title="Audiobook pending"><Volume2 size={14} /></span>}</div>; })}
    </nav>
